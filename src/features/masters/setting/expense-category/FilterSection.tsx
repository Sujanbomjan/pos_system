import useFetchExpenseCategory from "@/api/hooks/master/setting/expenseCategory/useFectchExpenseCategoryAll";
...
import { Button, Checkbox, Input, Popover, cn } from "rizzui";
...
const FilterSection = () => {
  ...
  const expenseCategoryData = useFetchExpenseCategory();

  //multiple delete logic
  const [loading, setLoading] = useState(false);
  const client = useQueryClient();
  const deleteSelected = async () => {
    if (expenseCategoryData.data?.data[0].data.length === 0) {
      toast.error("No items left to remove");
      return;
    }
    setLoading(true);
    try {
      const requests = expenseCategoryData.data?.data[0].data?.map(
        (item: any) => api.delete(getApiRoute("deleteExpenseCategory")(item.id))
      ) as any;
  ...
  const exportSelected = async () => {
    // const expenseCategoryItems = expenseCategoryData.data?.data[0].data || [];
    // if (!expenseCategoryItems || expenseCategoryItems.length === 0) {
    //   toast.error("No items to export");
    //   return;
    // }
    // exportToCSV(expenseCategoryItems, "Expense Category", "Expense Category");
  };
      ...
      <Popover shadow="sm" placement="bottom-end">
        ...
        <Popover.Content className="z-[9999] p-0 dark:bg-gray-100 [&>svg]:dark:fill-gray-100">
          <div className="flex flex-col">
            ...
            <Button variant="text" className="inline-flex gap-1">
              <CiImport size={20} />
              Import all
            </Button>
          </div>
