import useExportPurchase from "@/api/hooks/transaction/purchase/useExportPurchase";
import useFetchAllPurchase from "@/api/hooks/transaction/purchase/useFetchAllPurchase";
...
const FilterSection = () => {
  ...
  const purchaseData = useFetchAllPurchase();

  //multiple delete logic
  const [loading, setLoading] = useState(false);
  const client = useQueryClient();
  const deleteSelected = async () => {
    if (purchaseData.data?.data[0].data.length === 0) {
      toast.error("No items left to remove");
      return;
    }
    setLoading(true);
    try {
      const requests = purchaseData.data?.data[0].data?.map((item: any) =>
        api.delete(getApiRoute("deletePurchase")(item.id))
      ) as any;
  ...
  const { data: exportData, isLoading, isError, error } = useExportPurchase();
  ...
  return (
    <div className="flex flex-row gap-2 items-center mb-4">
      ...
      <FilterBuilder searchKey="searchByAnything" handleReset={handleReset}>
        {!isBS ? (
          <DateFiled
            className="w-full"
