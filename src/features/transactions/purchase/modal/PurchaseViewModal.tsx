            ...
            <div className="flex flex-row justify-between mt-6">
              <div className="flex-1">
                ...
                <strong className="text-lg">
                  {data.data?.data?.supplier?.name}
                </strong>
                <p className="mb-2">{data.data?.data?.supplier?.address}</p>
                <p>Tel: {data.data?.data?.supplier.phone}</p>
                <p>Email: {data.data?.data?.supplier.email}</p>
              </div>
              <div className="flex-1">
                <p className="mb-2">To:</p>
                <strong className="text-lg">
                  {data.data?.data?.warehouse?.name}
                </strong>
                <p className="mb-2">{data.data?.data?.warehouse?.address}</p>
                <p>Tel: {data.data?.data?.warehouse?.phone}</p>
                <p>Email: {data.data?.data?.warehouse?.email}</p>
              </div>
            </div>
            ...
            <div className="mt-6">
              <table className="min-w-full divide-y divide-gray-200 border-collapse border border-gray-300">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {data.data?.data.purchase_items.map(
                    (
                      { id, product, quantity, subtotal, net_unit_cost },
                      index
                    ) => (
                      <tr className="hover:bg-gray-100" key={id}>
                        ...
                        <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-500 border-r-[1px]">
                          {Number(net_unit_cost).toFixed(2)}
                        </td>
