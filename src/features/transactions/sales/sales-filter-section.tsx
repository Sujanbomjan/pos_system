import useExportSales from "@/api/hooks/transaction/sales/useExportSales";
...
const SalesFilterSection = () => {
  ...
  const deleteSelected = async () => {
    if (salesData.data?.data[0].data.length === 0) {
      toast.error("No items left to remove");
      return;
    }
    setLoading(true);
    try {
      const requests = salesData.data?.data[0].data?.map((item: any) =>
        api.delete(getApiRoute("deleteSales")(item.id))
      ) as any;
  ...
  const { data: exportData, isLoading, isError, error } = useExportSales();
